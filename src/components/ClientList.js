...
export default function ClientList({ clients, onUpdate, onDelete, onReplace }){
  ...
  const filtered = clients.filter(c => {
    ...
    if(onlyPending){
      const total = c.sessionsRequired * 1000;
      if(c.paidAmount >= total) return false;
    }
    return true;
  });
  ...
  return (
    <div className="client-list card" style={{marginTop:12}}>
      ...
      {filtered.map(client => (
        <ClientItem key={client.id} client={client} onUpdate={onUpdate} onDelete={onDelete} onReplace={onReplace} />
      ))}
    </div>
  );
}
